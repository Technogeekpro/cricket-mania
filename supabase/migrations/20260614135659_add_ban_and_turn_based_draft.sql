-- Admin profile controls and captain-driven one-by-one team draft.
...
alter table public.matches
  add column if not exists toss_winner text not null default 'a' check (toss_winner in ('a', 'b')),
  add column if not exists draft_turn text not null default 'a' check (draft_turn in ('a', 'b'));
